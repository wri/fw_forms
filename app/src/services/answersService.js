...
class AnswersService {
  static *getAllTemplateAnswers({ reportId, template, loggedUser, team, query }) {
    let filter = {};
  ...
  static *filterAnswersByArea({ reportId, template, loggedUser, team, query, areaId }) {
    let filter = {};
    let currentManager = false;
    const confirmedUsers = [];
    if (team) {
    ...
    if (loggedUser.role === "ADMIN" || loggedUser.id === template.user) {
      ...
      // managers can check all answers from the default template from his and his team's members
      filter = {
        $and: [{ report: new ObjectId(reportId) }, { user: new ObjectId(loggedUser.id) }, { areaOfInterest: areaId }]
      };
    }
    if (query) {
      Object.keys(query).forEach(key => {
