...
describe("Get report by id endpoint", function () {
  ...
  it("Getting report by id should return the report (happy case)", async function () {
    ...
    responseReport.attributes.should.have
      .property("languages")
      .and.be.an("array")
      .and.length(1)
      .and.contains(createdReport.languages[0]);
  ...
  it("Getting the default report should return it (happy case)", async function () {
    mockGetUserFromToken(ROLES.USER);
    const createdReport = await createReport({
      _id: config.get("defaultTemplateId")
    });

    const response = await requester.get(`/api/v1/reports/default`).set("Authorization", `Bearer abcd`).send();
  ...
  it("Getting the legacy report should return it (happy case)", async function () {
    mockGetUserFromToken(ROLES.USER);
    const createdReport = await createReport({
      _id: config.get("defaultTemplateId")
    });
