...
describe("Get reports tests", function () {
  ...
  it('Get all reports as an anonymous user should return an "Not logged" error with matching 401 HTTP code', async function () {
    const response = await requester.get(`/v3/reports`).send();
  ...
  it("Get all reports as an authenticated user should return an empty list", async function () {
    mockGetUserFromToken(ROLES.USER);
    const response = await requester.get(`/v3/reports`).set("Authorization", `Bearer abcd`).send();

    response.status.should.equal(200);
    response.body.should.have.property("data").and.be.an("array").and.length(0);
  });

  it("Get all reports should be successful and return a list of reports (populated db)", async function () {
    mockGetUserFromToken(ROLES.USER);
    ...
    responseReportOne.attributes.should.have
      .property("languages")
      .and.be.an("array")
      .and.length(1)
      .and.contains(reportOne.languages[0]);
    ...
    responseReportTwo.attributes.should.have
      .property("languages")
      .and.be.an("array")
      .and.length(1)
      .and.contains(reportTwo.languages[0]);
  ...
  it("Get all reports without filter should be successful and return a list of reports which belong to the user or are public and published (populated db)", async function () {
    mockGetUserFromToken(ROLES.USER);
